...
// REGEX
const nameRegex = /^([\u0621-\u064A]{3,15})?([-]{0,1})?([\u0621-\u064A]{3,15})$/;
const emailRegex = /^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/;
const phoneRegex = /^([\u0621-\u064A0-9 ]{3,15})?([-]{0,1})?([\u0621-\u064A0-9 ]{3,15})$/;
...
userphone.addEventListener('input', () => checkInput(userphone, phoneRegex));
...
form.addEventListener('submit', e => {
    ...
    if (userLastname.classList.contains('is-valid') && userFirstname.classList.contains('is-valid') && userEmail.classList.contains('is-valid') && userphone.classList.contains('is-valid') && userMessage.classList.contains('is-valid')) {
        sendEmail();
        showAlert('alert-success', 'تم ارسال حجزك');
        form.reset();
        allFormField.forEach(input => input.classList.remove('is-valid'));
    } else showAlert('alert-danger', 'من فضلك اعد تصحيح البيانات');
});
...
function sendEmail() {
    ...
    const serviceID = 'service_6dlyvxr';
    const templateID = 'template_b2dlo5r';
